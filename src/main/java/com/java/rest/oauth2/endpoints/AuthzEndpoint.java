...
public class AuthzEndpoint {
    ...
    public Response authorize(@Context HttpServletRequest request)
    	...
    	System.out.println("inside /authz");
        try {
            OAuthAuthzRequest oauthRequest = new OAuthAuthzRequest(request);
            OAuthIssuerImpl oauthIssuerImpl = new OAuthIssuerImpl(new MD5Generator());
            
            System.out.println("inside /authz getClientId: " + oauthRequest.getClientId());
            System.out.println("inside /authz client_secret : " + oauthRequest.getClientSecret());
            
            if (!database.verifyClientId(oauthRequest.getClientId())) {
            	
                return Response.status(401).entity("Invalid Client ID").build();
            }
